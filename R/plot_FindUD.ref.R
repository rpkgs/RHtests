    ...
    QMout, Mq, Ns, adj, adjB, Ips, Iseg.adj, ...)
{
     ...
     text(2, yrr1, "Adjustments estimated from Base", adj = 0, cex = .8)
     lines(dim(ori.bdata)[1] * c(.31, .33), rep(yrr1, 2), col = "red")
     text(2, yrr2, "Adjustments estimated from Diff", adj = 0, cex = .8)
     lines(dim(ori.bdata)[1] * c(.31, .33), rep(yrr2, 2), col = "blue")
     ...
     text(2, yrr1, "Adjustments estimated from Base", adj = 0, cex = .8)
     lines(dim(ori.bdata)[1] * c(.31, .33), rep(yrr1, 2), col = "black")
     text(2, yrr2, "Adjustments estimated from Diff", adj = 0, cex = .8)
     lines(dim(ori.bdata)[1] * c(.31, .33), rep(yrr2, 2), col = "red")
                     ...
                     if (col == 0) {
                         ...
                         for (j in 1:QMout$Mq) {
                             lines(c(osmean[(j + 1), icol] - Fd, osmean[(j + 1), icol] + Fd),
                                 c(rep(osmean[(j + 1), (icol + 1)], 2)),
                                 col = col, lty = 2, lwd = .5
                             )
                             if (j >= 1 & j < QMout$Mq) {
                                 lines(rep(osmean[(j + 1), icol] + Fd, 2),
                                     c(osmean[(j + 1), (icol + 1)], osmean[(j + 2), (icol + 1)]),
                                     col = col, lty = 2, lwd = .5
                                 )
                             }
                         }
