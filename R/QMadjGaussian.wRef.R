...
QMadjGaussian.wRef <- function(B, Dif, Ips, Mq, Iseg.adj, Nadj, Nt, Ny4a) {
      ...
      for (k in 2:(Mq + 1)) {
        ...
        if (sum(!is.na(Yd[iindx[(Mp1 + 1):Mp2]])) < Nmin.Mq) {
          if (Mq > 1) {
            ...
            ErrorMSG <<- paste(
              "!!!! Warning - The Ref series has too many missing values to do QM-adjustments.\n",
              "Only mean-adjustments were done. You may want to use a better Ref series if possible!\n",
              get("ErrorMSG", env = .GlobalEnv), "\n"
            )
          }
        } else {
          EBb[i, k] <- mean(Yd[iindx[(Mp1 + 1):Mp2]], na.rm = TRUE)
        }
      }
      EBb[i, 1] <- EBb[i, 2]
    ...
    if (tflg == FALSE) {
      ...
      for (i in 1:(Ns + 1)) {
        ...
        if (i == Iseg.adj) {
          ...
          if (remain < 0) {
            DB[DB < 0] <- DB[DB < 0] + remain * Nseg * DB[DB < 0] / sDBne
          } else {
            DB[DB > 0] <- DB[DB > 0] + remain * Nseg * DB[DB > 0] / sDBpo
          }
  ...
  listk(PA = BA, DB, Mq, Nseg.mn, osmean, osp, AdjM)
}
